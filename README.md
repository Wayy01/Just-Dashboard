...
| **Databases** | Postgres, MySQL, MongoDB and SQLite. A data grid that edits rows through forms — insert, edit and delete, always scoped to a primary key — beside a structure view of columns, indexes and foreign keys, a query runner that classifies a statement as destructive before it runs (with per-connection history and saved snippets), CSV/JSON export, and one-click generation of a Prisma or Drizzle schema from the live database. Plus dumps and restores. Passwords never appear in argv. |
