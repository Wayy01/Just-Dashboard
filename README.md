...
| **Proxy & TLS** | A form that puts a domain in front of a port and writes the nginx for you — TLS, HTTP/2, HSTS, WebSockets, upload limits, IP allow lists and basic auth — rendered on the server and shown live next to the form, so the file it produces is ordinary nginx you can commit and edit by hand. Certificates issued, renewed and revoked through certbot, with the renewal schedule reported because a stopped timer is the real story behind almost every expired certificate. A live TLS report grading what a visitor actually gets: protocol versions probed one at a time, the chain as presented, HSTS, the security headers and whether plain HTTP redirects. Raw config editing is still there, validated with the server's own test binary before writing or reloading. |
...
| **Security** | A verdict on the host, not just its settings: exposure, firewall, sshd, intrusion prevention, open ports, certificates and pending security patches, each finding carrying what was measured, what it means, what to do, and where the dashboard can do it, a button. Firewall rules with a named-service catalogue that warns *before* you open Redis to the world, plus the default policies and logging level that decide what the rule list actually means. sshd's own settings, applied through its parser and rolled back if it objects — and refused outright when the change would leave nobody a way in. fail2ban jails you can tune and allowlist, folded into the one question its ban list cannot answer: who keeps coming back. Live connections by peer, interfaces and routes, the host's login record, and ping, DNS, traceroute and port checks so the answer is on the page the question came from. |
