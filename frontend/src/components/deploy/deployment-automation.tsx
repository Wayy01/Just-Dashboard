...
export function DeploymentAutomation({ projectID, environmentID, legacyHook, legacyEnabled, normalized }: Props) {
  ...
  const saveTrigger = async () => {
    ...
    try {
      ...
      setNewSecret({ url: `/api/v1/hooks/providers/${provider}/${result.trigger.hookId}`, secret: result.secret })
  ...
  return (
    <div className="grid min-w-0 gap-4 xl:grid-cols-2">
      ...
      <Panel><PanelHeader icon={Clock} title="Scheduled actions" description="Schedules use an explicit timezone and retain their action chain." /><PanelBody>{schedules.loading && !schedules.data ? <LoadingPanel rows={2} /> : schedules.error ? <ErrorState error={schedules.error} /> : (schedules.data?.length ?? 0) === 0 ? <EmptyState icon={Clock} title="No schedules" description="Add schedules through the API while the guided editor is completed." className="border-0 py-5" /> : <div className="space-y-3">{schedules.data?.map((schedule) => <div key={schedule.id} className="rounded-lg border border-hairline p-3"><div className="flex justify-between gap-3"><p className="text-[13px] font-medium">{schedule.name}</p><Badge variant={schedule.enabled ? "default" : "secondary"}>{schedule.enabled ? "Enabled" : "Paused"}</Badge></div><p className="mt-1 font-mono text-[11px] text-muted-foreground">{schedule.expression} · {schedule.timezone}</p><p className="mt-1 text-xs text-muted-foreground">{schedule.nextRunAt ? `Next ${relativeTime(schedule.nextRunAt)}` : "No next run"} · {schedule.steps.length} actions</p></div>)}</div>}</PanelBody></Panel>
      ...
      <Panel className="xl:col-span-2"><PanelHeader icon={Bell} title="Notifications" /><PanelBody><Notice title="Signed delivery boundary active">Outbound channels sign the exact bounded JSON body and keep response classes—not response bodies—in delivery history. Channel management is available through the automation API.</Notice></PanelBody></Panel>
    </div>
  )
}
