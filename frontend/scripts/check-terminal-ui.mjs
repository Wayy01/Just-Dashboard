...
try {
  ...
  const fontFamily = page.getByRole("combobox", { name: "Font family", exact: true })
  await fontFamily.click()
  await page.getByRole("option", { name: "JetBrains Mono", exact: true }).click()
  const cursorStyle = page.getByRole("combobox", { name: "Cursor style", exact: true })
  await cursorStyle.click()
  await page.getByRole("option", { name: "Bar", exact: true }).click()
  await page.getByRole("slider", { name: "Line height", exact: true }).press("End")
  await page.getByRole("slider", { name: "Letter spacing", exact: true }).press("End")
  await page.getByRole("switch", { name: "Blinking cursor", exact: true }).click()
  const savedSettings = await page.evaluate(() => JSON.parse(localStorage.getItem("jd.terminal.settings")))
  assert.equal(savedSettings.fontFamily, '"JetBrains Mono", monospace')
  assert.equal(savedSettings.cursorStyle, "bar")
  assert.equal(savedSettings.lineHeight, 2)
  assert.equal(savedSettings.letterSpacing, 3)
  assert.equal(savedSettings.cursorBlink, false)
  await page.getByRole("button", { name: "Larger text", exact: true }).click()
  await settingsPanel.getByRole("button", { name: "Reset", exact: true }).click()
  assert.equal(await page.evaluate(() => localStorage.getItem("jd.terminal.settings")), null)
